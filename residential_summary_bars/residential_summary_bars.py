...
ax5.set_ylim(bottom=min(0, min(share_vals)) - y_span * 0.4)
...
alloc_colors = ["#8da0cb", "#66c2a5", "#fc8d62"]
...
ax6.set_ylabel("Land Allocation (% of Total Assessed Value)", fontsize=12)
...
fig8, (ax_top, ax_bot) = plt.subplots(2, 1, figsize=(8, 11), sharey=True)
...
for ax, vals, subtitle, note in [
    ...
    for bar, val in zip(bars, vals):
        ...
        va  = "bottom" if val >= 0 else "top"
        ax.text(
            bar.get_x() + bar.get_width() / 2, val + pad,
            f"{val:+.1f}%",
            ha="center", va=va,
...
fig9, (ax9_top, ax9_bot) = plt.subplots(2, 1, figsize=(8, 11), sharey=True)
